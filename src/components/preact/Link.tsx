...
export function Link({ children, class: className, type, to = '#' }: Props) {
    ...
    if (type === types.transparent || !type) {
        return (
            <a
                href={to}
                class={`w-fit font-light -tracking-wider hover:backdrop-brightness-50 transition-all duration-300 text-white px-10 py-2 backdrop-brightness-95 bg-transparent ${className}`}>
                {children}
            </a>
        )
    }
